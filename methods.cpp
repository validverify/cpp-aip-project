 ...
 *
 * @throw CAN_NOT_LOAD_IMAGE_FILE(LOAD_IMAGE_PIXELS:CAN_NOT_LOAD_IMAGE_FILE) - When image loader in stb_image can not
 * open image file;
 * @throw CAN_NOT_SAVE_IMAGE - When function save_result can not save image with redacted data throw stb_image module;
 *
 * @return In constructor function class returns own object with accessible(public) functions: get_image_loader,
 * get_image_params, get_pixels_range, save_result, free_space
 */
class BasicImage
{
  public:
 ...
 *
 * @throw TOO_MANY_SENSETIVE_DATA_TO_ENCODE - When encode function got too big sensetive data message even image can
 * hold this;
...
class ChannelSwapping
{
  public:
	...
	void encode(const std::string &img_path, const std::string &sens_data, const std::string &output_path)
	{
		...
		if (total_bits > (w * h))
		{
			std::cerr << "Error: TOO MANY SENSETIVE DATA TO ENCODE. R: Sensitive data to encode is too large for this "
						 "image file. Data: "
					  << total_bits << " - sens data size, " << pixels.size() << " - image pixels total size."
					  << std::endl;
			return;
		}
